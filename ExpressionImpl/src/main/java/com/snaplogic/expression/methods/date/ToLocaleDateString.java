 ...
 * @author mklumpp
 */
public enum ToLocaleDateString implements Method {
    INSTANCE;

    @Override
    public String evaluate(final Object member, final List args) {
        ...
        if (member instanceof DateTime) {
            ...
            if (params != null) {
                ...
                if (!StringUtils.isBlank(format)) {
                    return dateTime.toLocalDate().toString(format);
                }
                return dateTime.toLocalDate().toString();
            }
            return ((DateTime) member).toLocalDate().toString();
        }
