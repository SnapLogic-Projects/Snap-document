...
import static com.snaplogic.expression.methods.date.Messages.INVALID_DAY_OF_WEEK_RESOLUTION;

/**
 * Extension of the JavaScript Date object with this Joda method:
 * {@see http://joda-time.sourceforge.net/apidocs/org/joda/time/DateTime.html#withDayOfWeek(int)}
 *
 * @author tstack
 */
public enum WithDayOfWeek implements Method {
    INSTANCE;
    ...
    public Object evaluate(final Object member, final List args) {
        ...
        try {
            return dateTime.withDayOfWeek(MethodUtils.getArgAsNumber(args, 0).intValue());
        } catch (IllegalFieldValueException e) {
            throw new SnapDataException(e, INVALID_ARGUMENT)
                    .withReason(e.getMessage())
                    .withResolution(INVALID_DAY_OF_WEEK_RESOLUTION);
        }
    }
}
